    ...
    function initDropdowns() {
      ...
      if (!isTouch) {
        document.querySelectorAll('.dropdown').forEach(function (dd) {
          ...
          dd.addEventListener('mouseleave', function () {
            leaveTimer = setTimeout(function () { dd.classList.remove('open'); }, 150);
          });
        });
      }
      ...
      document.addEventListener('click', function (e) {
        if (!e.target.closest('.dropdown') && !e.target.closest('#apps-popup')) {
          closeAllDrops();
        }
      });
    }
    ...
    document.addEventListener('click', function (e) {
      if (e.target.classList.contains('drop-item')) { document.querySelectorAll('.dropdown').forEach(d => d.classList.remove('open')); return; }
